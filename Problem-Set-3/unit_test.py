...
EXERCISE_OR_ASSIGNMENT_NAME = "exercise3"
...
class TestTree(unittest.TestCase):
    def test_attributes(self):
        self.assertCountEqual(["info", "next"],
                              vars(Link(5)).keys(),
                              msg=f"The class Node does not have the right attributes!")
        self.assertCountEqual(["info", "next", "prev"],
                              vars(DLink(5)).keys(),
                              msg=f"The class Node does not have the right attributes!")
        self.assertCountEqual(["start"],
                              vars(BasicLinkedList()).keys(),
                              msg=f"The class Node does not have the right attributes!")
        self.assertCountEqual(["start", "end"],
                              vars(BasicDLinkedList()).keys(),
                              msg=f"The class Node does not have the right attributes!")

    def test_methods(self):
        methods = ['pop', 'append']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(QueueSingle)),
                              msg=f"The class BinarySearchTreeDouble does not have the right methods!")
        methods = ['pop', 'append']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(Queue)),
                              msg=f"The class BinarySearchTreeDouble does not have the right methods!")
        methods = ['pop', 'insert', 'search']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(SingleLinkedList)),
                              msg=f"The class BinarySearchTreeDouble does not have the right methods!")
        methods = ['pop', 'insert', 'search']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(DoubleLinkedList)),
                              msg=f"The class BinarySearchTreeDouble does not have the right methods!")
