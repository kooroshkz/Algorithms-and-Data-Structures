...
EXERCISE_OR_ASSIGNMENT_NAME = "exercise2"
...
class TestTree(unittest.TestCase):
    def test_attributes(self):
        s = Node(3)
        self.assertCountEqual(["info", "left", "right"],
                              vars(s).keys(),
                              msg=f"The class Node does not have the right attributes!")
        s = NodeD(3)
        self.assertCountEqual(["info", "parent", "left", "right"],
                              vars(s).keys(),
                              msg=f"The class Node does not have the right attributes!")
        s = BinaryTree()
        self.assertCountEqual(["root"],
                              vars(s).keys(),
                              msg=f"The class BinaryTree does not have the right attributes!")
        ...
        s = BinarySearchTreeDouble()
        self.assertCountEqual(["root"],
                              vars(s).keys(),
                              msg=f"The class BinaryTree does not have the right attributes!")

    def test_methods(self):
        methods = ['add', 'count_leafs', 'count_subtree_leafs', 'get_height', 'get_highest_subtree_value',
                   'get_highest_value', 'get_subtree_height', 'show']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(BinaryTree)),
                              msg=f"The class BinaryTree does not have the right methods!")
        methods = ['search', 'add', 'get_highest_value', 'get_lowest_value', 'is_binary_search_tree', 'remove']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(BinarySearchTree)),
                              msg=f"The class BinarySearchTree does not have the right methods!")
        methods = ['search', 'add', 'remove']
        self.assertCountEqual(methods,
                              filter(lambda x: x[0] != "_",  vars(BinarySearchTreeDouble)),
                              msg=f"The class BinarySearchTreeDouble does not have the right methods!")
